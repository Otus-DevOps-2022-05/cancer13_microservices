...
Использовал:
  ...
  Самописный chart с mongodb v3.2
  GitLab 15.5

Не удалось реализовать поднятие окружения для "разработчиков" по комиту, и деление по стендам.
Поддержка kubernetes Deprecated in GitLab 14.5, но общую концепция я понял, и можно было бы на тот же кластер деплоить через ансибл или установив на ранера приложения и подсунув .kube/config (как если бы мы делали это вручную)

Замечания:
  Для работы с gitlab helm repo нужен VPN(
  GitLab с указанием <EXT_IP-адресс_ingress-контроллера>.sslip.io в global.hosts.domain
  ...
  Докер жалуется на не безопасность использования параметра -p, останавливает работу, рекомендует использовать --password-stdin, переделал:
